...
# Store the names of those movies in last_10 vector (in the same order)
	...
	last_10 <- tail(bollywood, 10)$Movie
...
# Store the movie name in variable named top_movie
  ...
  top_movie <- as.character(bollywood %>% select(Movie, Tcollection) %>% arrange(desc(Tcollection)) %>% top_n(1) %>% select(Movie) %>% mutate(Movie = as.character(Movie)))
...
# Store the movie name in variable named top_2_movie
  ...
  top_2_movie <- as.character(bollywood %>% select(Movie, Tcollection) %>% arrange(desc(Tcollection)) %>% top_n(2) %>% 
                                select(Movie) %>% tail(1) %>% mutate(Movie = as.character(Movie)))
...
# You can use	a column named 'Tcollection' for this 
  ...
  shahrukh_collection <- as.double(shahrukh %>% group_by(Lead) %>% summarise(Total = sum(Tcollection)) %>% select(Total))
    
	akshay_collection <- as.double(akshay %>% group_by(Lead) %>% summarise(Total = sum(Tcollection)) %>% select(Total))
    
	amitabh_collection <- as.double(amitabh %>% group_by(Lead) %>% summarise(Total = sum(Tcollection)) %>% select(Total))
...
verdict_count <- bollywood %>% group_by(Verdict) %>% summarise(Count = n())
...
col_names <- grep('collection', names(bollywood), value = T)
movie_result <- col_names %>% map_chr(.f  = function(x){
                    
                    as.character(bollywood %>% arrange_(.dots = x) %>% filter_(.dots = paste0("!is.na(","Fwcollection)"))%>% tail(1) %>% select(Movie)  %>% 
